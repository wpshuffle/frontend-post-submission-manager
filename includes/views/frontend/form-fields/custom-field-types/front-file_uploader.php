...
if (is_array($custom_field_saved_value)) {
    $media_id_array = $custom_field_saved_value;
} else {
...
<div class="fpsm-file-preview-wrap">
    ...
    ?>
            <div class="fpsm-file-preview-row" data-media-id="<?php echo esc_attr($media_id); ?>">
                ...
                <span class="fpsm-file-preview-column"><input type="button" class="fpsm-media-delete-button" data-media-id="<?php echo intval($media_id); ?>" data-media-key="<?php echo esc_attr($attachment_code); ?>" value="<?php esc_attr_e('Delete', 'frontend-post-submission-manager'); ?>" data-edit="<?php echo (!empty($edit_post)) ? 'yes' : 'no'; ?>"></span>
            </div>
    <?php
        }
    }
