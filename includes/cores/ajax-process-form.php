...
if ( $this->admin_ajax_nonce_verify() ) {
    ...
    if ( $form_row->form_type == 'login_require' && !empty( $form_details['basic']['limit_post_submission'] ) && !empty( $form_details['basic']['allowed_number_posts'] ) ) {
        ...
        if ( is_user_logged_in() ) {
            ...
            $author_total_posts = count_user_posts( $post_author_id, $form_row->post_type );
...
//            if (!empty($post_id)) {
//                $comment_status = (comments_open($post_id)) ? 'open' : 'closed';
//            } else {
//                $comment_status = get_option('default_comment_status');
//            }
//            $postarr['comment_status'] = $comment_status;
