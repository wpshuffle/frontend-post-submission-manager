...
| --- | --- | --- | --- |
| PHP syntax lint | PHP 7.4 | All PHP files parse without errors | Passed in GitHub Actions run 32754002423 |
| PHP syntax lint | PHP 8.3 | All PHP files parse without errors | Passed in GitHub Actions run 32754002423 |
...
| --- | --- | --- | --- |
| QS-01 | Fresh install or clean test state; sign in as administrator; open **FPSM → Forms** with no forms. | One four-step Quick Start panel appears. Steps read Create Form, Add Fields, Publish Shortcode, and Test Submission. | Not run |
...
| QS-04 | With no forms, activate **Create your first form**. | The existing Add New Form route opens once; no activation redirect or extra remote request occurs. | Not run |
...
| QS-08 | Submit the dismissal request with an invalid or missing nonce. | The request is rejected; no user preference is written; forms/settings remain unchanged. | Not run |
...
| QS-11 | Inspect at desktop, approximately 768px, and 375px widths at 100% and 200% zoom. | Content remains readable without clipped controls or unintended horizontal scrolling. | Not run |
