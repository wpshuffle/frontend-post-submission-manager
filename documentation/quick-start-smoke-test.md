...
| Isolated runtime smoke test | WordPress 7.1 / PHP 8.3, WordPress Playground CLI 3.1.51 | Functional, security, persistence, activation, and rollback behaviors match the matrix | 11 scenarios passed and 4 are partial on 2026-08-28; details below |
...
| QS-10 | Use keyboard only: Tab through the panel, activate each link, and dismiss. | Focus order is logical, every control is reachable, and focus is visibly indicated. | **Partial** — native focusable controls and focus CSS confirmed; keyboard traversal unrun because no browser binary was available |
| QS-11 | Inspect at desktop, approximately 768px, and 375px widths at 100% and 200% zoom. | Content remains readable in locally bundled Urbanist without clipped controls, unintended horizontal scrolling, or remote font requests. | **Partial** — scoped CSS and local WOFF2 returned successfully; responsive/zoom rendering unrun because no browser binary was available |
| QS-12 | Run QS-01 through QS-11 with `WP_DEBUG` enabled and inspect the browser console. | No PHP warning/notice and no JavaScript console error is introduced. | **Partial** — captured PHP 7.4/8.3 HTML and server logs were clean with `WP_DEBUG`; browser-console inspection unrun |
| QS-13 | Upgrade an existing FPSM installation containing saved forms to this branch. Open, edit, and submit an existing form. | Existing forms, options, shortcodes, hooks, and submission behavior continue unchanged; Quick Start remains absent. | **Partial** — existing rows and edit screen loaded with Quick Start absent; edit-save and frontend submission remain unrun |
...
Runtime evidence in this document records assertions made against an ephemeral local WordPress Playground instance. Cookies and raw server responses are intentionally not committed. Browser screenshots, console output, keyboard traversal, responsive/zoom inspection, edit-save, frontend submission, and a short implementation recording remain required before the release gate can be considered complete.
